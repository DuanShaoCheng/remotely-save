...
## Fork 修复内容 (v0.5.25-fix.3)

本 fork 在原版 v0.5.25 基础上包含以下稳定性改进：

1. **指数退避重试** — WebDAV、S3、OneDrive 后端遇到 HTTP 429/500/502/503/504 错误时自动重试，带指数退避和随机抖动。
2. **最小同步间隔** — 30 秒最小间隔防止 sync-on-save 触发的快速循环同步。
3. **文件名校验** — 跨平台文件名校验以警告模式重新启用；Windows 严格模式仍会阻止同步。
4. **加密重上传修复** — `fullfillMTimeOfRemoteEntityInplace` 现在在上传后正确传播 mtime，避免加密文件不必要的重上传。
5. **同步触发范围** — 文件变更事件被累积并过滤，跳过未启用同步的配置目录变更。
6. **ArrayBuffer 兼容性** — `bufferToArrayBuffer` 创建全新的 ArrayBuffer，避免 SharedArrayBuffer 类型问题。
7. **凭据泄露防护** — `plugins/remotely-save/data.json` 在同步时始终被跳过，防止凭据泄露。

### 重要：构建说明

**本 fork 必须从[上游原仓库](https://github.com/remotely-save/remotely-save)源码构建**，而不是从 fork 自身的 `npm install`。直接 fork 后运行 `npm install` 可能解析到不同的依赖版本，导致打包产物不同，破坏 OAuth 认证（出现 `ERR_HTTP2_PROTOCOL_ERROR`）。

正确构建步骤：
```bash
git clone https://github.com/remotely-save/remotely-save.git
cd remotely-save
git apply fixes.patch   # 应用 fork 的源码修改
npm install
npm run build           # 使用 webpack，不要用 esbuild
```
